...
class OrderAO extends BaseAO {
    ...
    public function test_xa_transatction($user_id) {
        if (empty($user_id)) return false;
        $uuid = cls_rollrand::get_uuid();
        $tx_params = array('user_id' => $user_id);

        $this->addDistributeDbroute($this->orderModel->getDbroute(),$uuid,$tx_params);
        $this->addDistributeDbroute($this->cityModel->getDbroute(),$uuid);
        try {
            $this->xa_start($uuid);

            $order_id = $this->orderModel->insert($user_id);
            $is_update = $this->cityModel->insert();

            $this->xa_end_prepare_and_commit($uuid);
            echo "order_id id is " . $order_id . " " . "order_goods_id is " . "city is_update:" . $is_update;
            return true;
        } catch (Exception $e) {
            $this->xa_rollback($uuid);
            echo "exception for xa_rollback " . $e->getMessage();
            return false;
        }
    }
