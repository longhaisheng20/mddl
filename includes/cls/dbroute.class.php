...
class cls_dbroute {
	...
	private function setConnection($params = array()) {
        $db = $this->getDateBaseName($params);
		$this->setDBConn($db);
		return $db;
	}
    ...
    public function xa_start($uuid,$params = array()){
    	$db_name = $this->setConnection($params);
    	if ($this->getDbParse()->getIsSingleDb()) {
			$this->getSingleConnection()->xa_start($uuid);
		} else {
			$db_name = $this->setConnection($params);
			$this->getDbConnection($db_name)->xa_start($uuid);
		}
    }
    
    public function xa_end_prepare($uuid,$params = array()){
       	if ($this->getDbParse()->getIsSingleDb()) {
			$this->getSingleConnection()->xa_end_prepare($uuid);
		} else {
			$db_name = $this->setConnection($params);
			$this->getDbConnection($db_name)->xa_end_prepare($uuid);
		}
    }
    
    public function xa_commit($uuid,$params = array()){
        if ($this->getDbParse()->getIsSingleDb()) {
			$this->getSingleConnection()->xa_commit($uuid);
		} else {
			$db_name = $this->setConnection($params);
			$this->getDbConnection($db_name)->xa_commit($uuid);
		}
    }
    
    public function xa_rollback($uuid,$params = array()){
        if ($this->getDbParse()->getIsSingleDb()) {
			$this->getSingleConnection()->xa_rollback($uuid);
		} else {
			$db_name = $this->setConnection($params);
			$this->getDbConnection($db_name)->xa_rollback($uuid);
		}
    }
    ...
    public function getDateBaseName($params) {
        $logicTable = $this->getDbParse()->getLogicTable();
        $db = null;
        if ($logicTable) {
